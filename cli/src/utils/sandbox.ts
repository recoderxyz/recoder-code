...
import type { Config, SandboxConfig } from 'recoder-code-core';
import { FatalSandboxError } from 'recoder-code-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'recoder-code-sandbox';
const SANDBOX_NETWORK_NAME = 'recoder-code-sandbox';
const SANDBOX_PROXY_NAME = 'recoder-code-sandbox-proxy';
...
export async function start_sandbox(
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      // start and set up proxy if RECODER_SANDBOX_PROXY_COMMAND is set
      const proxyCommand = process.env['RECODER_SANDBOX_PROXY_COMMAND'] ?? process.env['GEMINI_SANDBOX_PROXY_COMMAND'];
    ...
    // RECODER_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env['RECODER_SANDBOX_PROXY_COMMAND'] ?? process.env['GEMINI_SANDBOX_PROXY_COMMAND'];
