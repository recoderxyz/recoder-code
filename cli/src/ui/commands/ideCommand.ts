...
import type { Config, IdeClient, File } from 'recoder-code-core';
import {
  RECODER_COMPANION_EXTENSION_NAME,
  getIdeInstaller,
  IDEConnectionStatus,
  ideContext,
    ...
    return {
      ...
      action: (): SlashCommandActionReturn =>
        ({
          type: 'message',
          messageType: 'error',
          content: `IDE integration is not supported in your current environment. To use this feature, run Recoder Code in one of these supported IDEs: VS Code or VS Code forks.`,
        }) as const,
    };
  }
    ...
    action: async (context) => {
      ...
      if (!installer) {
        context.ui.addItem(
          {
            type: 'error',
            text: `No installer is available for ${ideClient.getDetectedIdeDisplayName()}. Please install the '${RECODER_COMPANION_EXTENSION_NAME}' extension manually from the marketplace.`,
          },
          Date.now(),
        );
        return;
