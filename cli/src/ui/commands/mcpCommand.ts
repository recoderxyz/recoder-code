...
import type { DiscoveredMCPPrompt } from 'recoder-code-core';
import {
  DiscoveredMCPTool,
...
const getMcpStatus = async (
  ...
  if (serverNames.length === 0 && blockedMcpServers.length === 0) {
    const docsUrl =
      'https://github.com/arhansubasi/recoder-code/blob/main/docs/mcp-server.md';
    return {
      type: 'message',
      messageType: 'info',
...
const authCommand: SlashCommand = {
  ...
  action: async (
    ...
    try {
      ...
      const { MCPOAuthProvider } = await import('recoder-code-core');
