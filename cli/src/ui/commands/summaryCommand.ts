...
import { getProjectSummaryPrompt } from 'recoder-code-core';
import type { HistoryItemSummary } from '../types.js';

export const summaryCommand: SlashCommand = {
  name: 'summary',
  description:
    'Generate a project summary and save it to .recoder/PROJECT_SUMMARY.md',
  ...
  action: async (context): Promise<SlashCommandActionReturn> => {
    ...
    try {
      ...
      // Ensure .recoder directory exists
      const projectRoot = config.getProjectRoot();
      const recoderDir = path.join(projectRoot, '.recoder');
      try {
        await fsPromises.mkdir(recoderDir, { recursive: true });
      } catch (_err) {
        // Directory might already exist, ignore error
      }

      // Save the summary to PROJECT_SUMMARY.md
      const summaryPath = path.join(recoderDir, 'PROJECT_SUMMARY.md');
...
`;
      ...
      const completedSummaryItem: HistoryItemSummary = {
        ...
        summary: {
          isPending: false,
          stage: 'completed',
          filePath: '.recoder/PROJECT_SUMMARY.md',
        },
      };
