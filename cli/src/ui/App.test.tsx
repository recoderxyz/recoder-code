...
// Mock recoder-code-core and its Config class
vi.mock('recoder-code-core', async (importOriginal) => {
  const actualCore =
    await importOriginal<typeof import('recoder-code-core')>();
  const ConfigClassMock = vi
    ...
    .mockImplementation((optionsPassedToConstructor) => {
      ...
      return {
        ...
        getAllGeminiMdFilenames: vi.fn(() => ['RECODER.md']),
...
const { isGitRepository: mockedIsGitRepository } = vi.mocked(
  await import('recoder-code-core'),
);
...
describe('App UI', () => {
  ...
  describe('handleAutoUpdate', () => {
    ...
    it('should not start the update process when running from git', async () => {
      ...
      const info: UpdateObject = {
        ...
        message: 'Recoder Code update available!',
      };
  ...
  it('should display active file and other context', async () => {
    ...
    mockConfig.getAllGeminiMdFilenames.mockReturnValue(['RECODER.md']);
    ...
    await Promise.resolve();
    expect(lastFrame()).toContain(
      'Using: 1 open file (ctrl+g to view) | 1 RECODER.md file',
    );
  });

  it('should display default "RECODER.md" in footer when contextFileName is not set and count is 1', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(1);
    mockConfig.getAllGeminiMdFilenames.mockReturnValue(['RECODER.md']);
    ...
    expect(lastFrame()).toContain('Using: 1 RECODER.md file');
  });

  it('should display default "RECODER.md" with plural when contextFileName is not set and count is > 1', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(2);
    mockConfig.getAllGeminiMdFilenames.mockReturnValue(['RECODER.md', 'RECODER.md']);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
    ...
    currentUnmount = unmount;
    await Promise.resolve();
    expect(lastFrame()).toContain('Using: 2 RECODER.md files');
  });
  ...
  it('should display RECODER.md and MCP server count when both are present', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(2);
    mockConfig.getAllGeminiMdFilenames.mockReturnValue(['RECODER.md', 'RECODER.md']);
  ...
  it('should display only MCP server count when RECODER.md count is 0', async () => {
    mockConfig.getGeminiMdFileCount.mockReturnValue(0);
    mockConfig.getAllGeminiMdFilenames.mockReturnValue([]);
    mockConfig.getMcpServers.mockReturnValue({
      server1: {} as MCPServerConfig,
