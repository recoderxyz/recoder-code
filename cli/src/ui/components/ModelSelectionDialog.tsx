...
export const ModelSelectionDialog: React.FC<ModelSelectionDialogProps> = ({
  ...
  const options: Array<RadioSelectItem<string>> = availableModels.map(
    (model) => {
      const visionIndicator = model.isVision ? ' 👁️' : '';
      const freeIndicator = model.isFree ? ' 💎' : '';
      const contextIndicator = model.contextLength ? ` [${model.contextLength}]` : '';
      const currentIndicator = model.id === currentModel ? ' ⭐' : '';
      return {
        label: `${model.label}${visionIndicator}${freeIndicator}${contextIndicator}${currentIndicator}`,
        value: model.id,
      };
    },
  );
