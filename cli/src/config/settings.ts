...
export const SETTINGS_DIRECTORY_NAME = '.recoder';
...
export function getSystemSettingsPath(): string {
  ...
  if (platform() === 'darwin') {
    return '/Library/Application Support/RecoderCode/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\recoder-code\\settings.json';
  } else {
    return '/etc/recoder-code/settings.json';
  }
}
...
export function loadSettings(workspaceDir: string): LoadedSettings {
  ...
  // Apply built-in default MCP servers
  // These will be overridden by user/workspace/system settings as needed
  const defaultMcpServers = getDefaultMcpServers();
  systemDefaultSettings.mcpServers = {
    ...defaultMcpServers,
    ...(systemDefaultSettings.mcpServers || {}),
  };
