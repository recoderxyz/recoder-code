...
import { isExpensiveModel, formatCost } from '../../utils/modelPricing.js';
...
export class ContentGenerationPipeline {
  ...
  /**
   * Track cost of completed request
   */
  private trackCost(response: any): void {
    ...
    if (promptTokens > 0 || completionTokens > 0) {
      const record = costTracker.trackRequest(
        this.contentGeneratorConfig.model,
        promptTokens,
        completionTokens,
        'chat',
      ...
      // Show cost warning for expensive requests with cache info
      if (record.cost > 0.10) {
        let costMessage = `\n💰 Request cost: ${formatCost(record.cost)} ` +
          `(${promptTokens.toLocaleString()} prompt + ${completionTokens.toLocaleString()} completion tokens)`;
        
        // Add cache savings information if applicable
        if (cachedTokens > 0) {
          // Cached tokens typically cost 90% less (1/10th the price)
          // Estimate savings by calculating what those tokens would have cost
          const cacheSavingsRatio = 0.9;
          const pricing = isExpensiveModel(this.contentGeneratorConfig.model);
          const estimatedSavings = pricing 
            ? record.cost * (cachedTokens / Math.max(promptTokens, 1)) * cacheSavingsRatio
            : 0;
          
          costMessage += `\n   ⚡ Cache hit: ${cachedTokens.toLocaleString()} tokens cached` +
            (estimatedSavings > 0 ? ` (saved ~${formatCost(estimatedSavings)})` : ' (90% cost reduction)');
        }
        
        console.warn(costMessage + '\n');
      } else if (cachedTokens > 0) {
        // Show cache info even for inexpensive requests
        console.log(
          `⚡ Cache hit: ${cachedTokens.toLocaleString()} cached tokens (90% cost reduction)\n`,
        );
      }

      // Check if approaching budget limits
      const budgetCheck = costTracker.wouldExceedBudget(0);
      if (budgetCheck.wouldExceed) {
        console.warn(
          `\n⚠️  BUDGET ALERT: You've reached your ${budgetCheck.reason} limit!\n` +
          `Current: ${formatCost(budgetCheck.currentCost)} / Limit: ${formatCost(budgetCheck.limit)}\n`,
        );
      }
    }
  }
